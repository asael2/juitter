...
OBS: jQuery Masonry Integration after 'conectaTwitter.success'; Just call the juitterMasonry() function. (of course you need to implement this function in your script).
	...
	$.Juitter = {
		...
		conectaTwitter: function(e){
			...
			$.ajax({
				...
				timeout: 1000,
				error: function(){ 
					$("#"+contDiv).html("<div id='error'><p class='box'>Serviço temporariamente ocupado, tente <a href='#' onclick='location.reload(); return false;' title='Recarregar'>recarregar</a> a página.</p></div>");
				...
				success: function(json){
					...
					$.each(json.results,function(i,item) {
						if(e==1 || (i<numMSG && item.id>ultID)){
							...
							if (item.text != "undefined") {
								...
								if(fromID=="image") mHTML="<h2 class='author'><a href='http://www.twitter.com/"+item.from_user+"' target='_blank'>@"+item.from_user+"</a></h2><p class='tweet-cont'><a href='http://www.twitter.com/"+item.from_user+"' target='_blank'><img src='"+item.profile_image_url+"' alt='"+item.from_user+"' class='juitterAvatar' /></a><span class='tweet'>"+$.Juitter.textFormat(tweet)+"<span class='tweet-time'>"+item.created_at+"</span><a href='" + link + "' class='JRM' "+openLink+" target='_blank'>"+readMore+"</a></span></p>";
								...
								$("<li></li>")
									.append()
									.html("<div class='twittCont box'>"+mHTML+"</div>")  
					...
					//MASONRY ADDED HERE - LUCASVST
					if(juitterMasonry) juitterMasonry();
					//MASONRY ADDED HERE - LUCASVST
		...
		textFormat: function(texto){
			...
			var exp = /[\#]+([A-Za-z0-9-_]+)/ig;
			texto = texto.replace(exp,"<a href='' onclick='$.Juitter.start({searchType:\"searchWord\",searchObject:\"$1\"});return false;' class='hashLink'>#$1</a>"); 
			// make it bold
			if(mode=="searchWord"){
